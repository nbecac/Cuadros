...
const rowToProduct = (row: any): Product => ({
  ...
  tamanoTarjeta: row.tamano_tarjeta ?? 'mediano',
  proporcionTarjeta: row.proporcion_tarjeta ?? 'vertical',
  ajusteImagen: row.ajuste_imagen ?? 'cover',
  posicionImagen: row.posicion_imagen ?? 'center',
  tamanoModal: row.tamano_modal ?? 'normal',
  orden: row.orden,
...
const productToRow = (p: Partial<Product> & { id?: string }) => ({
  ...
  ...(p.tamanoTarjeta !== undefined && { tamano_tarjeta: p.tamanoTarjeta }),
  ...(p.proporcionTarjeta !== undefined && { proporcion_tarjeta: p.proporcionTarjeta }),
  ...(p.ajusteImagen !== undefined && { ajuste_imagen: p.ajusteImagen }),
  ...(p.posicionImagen !== undefined && { posicion_imagen: p.posicionImagen }),
  ...(p.tamanoModal !== undefined && { tamano_modal: p.tamanoModal }),
